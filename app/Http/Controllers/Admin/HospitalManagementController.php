...
class HospitalManagementController extends Controller
{
    ...
    $totalPatientDetails = PatientDetails::where('hospital_id', $user->id)->count();

    $todayPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereDate('created_at', $todayDate)->count();
    $thisMonthPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereMonth('created_at', $thisMonth)->count();
    $thisYearPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereYear('created_at', $thisYear)->count();

    $totalPatientDetailsadiscountamount = PatientDetails::where('hospital_id', $user->id)->sum('paitent_discount_amount');
    $totalDayPatientDetailsadiscountamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereDate('created_at', $todayDate)->sum('paitent_discount_amount');
    $totalMonthsPatientDetailsadiscountamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereMonth('created_at', $thisMonth)->sum('paitent_discount_amount');
    $totalYearPatientDetailsadiscountamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereYear('created_at', $thisYear)->sum('paitent_discount_amount');

    $totalPatientdischargeamount = PatientDetails::where('hospital_id', $user->id)->sum('after_discount_finall_bill');
    $totalDayPatientdischargeamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereDate('created_at', $todayDate)->sum('after_discount_finall_bill');
    $totalMonthsPatientdischargeamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereMonth('created_at', $thisMonth)->sum('after_discount_finall_bill');
    $totalYearPatientdischargeamount = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereYear('created_at', $thisYear)->sum('after_discount_finall_bill');


    $totalcompanycommission = PatientDetails::where('hospital_id', $user->id)->sum('company_commission_amount');
    $totalDaycompanycommission = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereDate('created_at', $todayDate)->sum('company_commission_amount');
    $totalMonthscompanycommission = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereMonth('created_at', $thisMonth)->sum('company_commission_amount');
    $totalYearcompanycommission = DB::table('add_paitent_details')->where('hospital_id', $user->id)->whereYear('created_at', $thisYear)->sum('company_commission_amount');

}

        return view('admin.hospital.hospital_dashboard')->with(compact(
            'totalPatientDetails',
            'todayPatientDetails',
    ...
    {
        $hospitalList = HospitalManagements::get()->toArray();
        return view('admin.hospital.hospital_list')->with(compact('hospitalList'));
    }

    public function ViewHospitalDetails($id)
    {
        $createhospital = HospitalManagements::find($id);
        $state = DB::table('states')->where('status', '1')->get();
        return view('admin.hospital.view_hospital_details')->with(compact('createhospital', 'state'));
    }


    public function AddEditHospital(Request $request, $id = null)
    {
        $request->all();
        Session::put('page', 'hospital-List');
        DB::beginTransaction();
        if ($id == "") {
            $title = "Add";
            ...
            if ($request->isMethod('post')) {
                ...
                $createhospital->green_card_discount = $data['green_card_discount'];
                $createhospital->silver_card_discount = $data['silver_card_discount'];
                $createhospital->gold_card_discount = $data['gold_card_discount'];
                $createhospital->company_discount = $data['company_discount'];
                $createhospital->member_id = $spom;
                $createhospital->created_by = Auth::guard('admin')->user()->id;
                $createhospital->status = 1;
                $createhospital->save();
                ...
                return redirect('admin/hospital-List')->with('success_message', $message);
            }
        } else {

            $title = "Edit ";
            $createhospital = HospitalManagements::find($id);
            $admin =  Admin::where('hospital_id', '=', $id)->first();
            ...
            if ($request->isMethod('post')) {
                ...
                $createhospital->gold_card_discount = $data['gold_card_discount'];
                $createhospital->company_discount = $data['company_discount'];
                $createhospital->updated_by = Auth::guard('admin')->user()->id;
                $createhospital->status = 1;
                $createhospital->save();
        ...
        $state = DB::table('states')->where('status', '1')->get();
        return view('admin.hospital.add_edit_hospital')->with(compact('title', 'createhospital', 'state'));
    }
    ...
    public function DeleteHospital($id)
    {
        ...
        $deleted_data = DB::table('create_hospital')->where('id', $id)->first();
        try {
            DB::table('create_hospital')->where('id', $id)->delete();
            DB::table('admins')->where('hospital_id', $id)->delete();
        } catch (ModelNotFoundException $exception) {
            return back()->withError($exception->getMessage())->withInput();
        }

        return redirect('/admin/hospital-List')->withErrors([' Data Deleted successfully ']);
    }

    public function ChangeHospitalListStatus(Request $request)
    {
        Session::put('page', 'hospital-List');
        $status_id = $request->get('status_id');

        $statuschange = DB::table('create_hospital')
            ->where('id', $status_id)
            ->first();

        DB::table('create_hospital')
            ->where('id', $status_id)
            ->update(array(
        ...
        DB::table('admins')
            ->where('hospital_id', $status_id)
            ->update(array(
                'updated_at' => date('Y-m-d H:i:s'),
                'status' => $request->get('status')
            ));
        $message = " Hospital Status Is  Updated Sucessfully!.";
        return redirect('admin/hospital-List')->with('success_message', $message);
    }

    public function HealthCardCustomerDetails()
    {
        $tabledata = DB::table('create_health_card')->get()->take(0);
        return view('admin.hospital.search_healthcard_customer')->with(compact('tabledata'));
    }

    public function HealthCardCustomerDetailSearch(Request $request)
    {
    ...
    public function HealthCardCustomerDetailsview($id)
    {
        $tabledata = IssueHealthCard::join('cities', 'cities.id', '=', 'create_health_card.assign_city')
            ...
            ->find($id);
        // dd($tabledata);
        return view('admin.hospital.search_healthcard_customer_view')->with(compact('tabledata'));
    }

    public function paitentlist()
    {
        $paitentlist = PatientDetails::where('hospital_id', Auth::guard('admin')->user()->id)->where('add_paitent_details.paitent_discharge_date', '=', null)->get();
        return view('admin.hospital.paitent_list')->with(compact('paitentlist'));
    }
    public function paitentdishargelist()
    {
        $paitentlist = PatientDetails::where('hospital_id', Auth::guard('admin')->user()->id)->where('add_paitent_details.paitent_discharge_date', '!=', null)->get();
        return view('admin.hospital.paitent_discharge_list')->with(compact('paitentlist'));
    }
    public function paitentdishargelistdetails($id)
    {
        $dischargepatientdetails = PatientDetails::find($id);
        return view('admin.hospital.paitent_dishargelist_details')->with(compact('dischargepatientdetails'));
    }


    public function AddPaitentDetails(Request $request, $id = null)
    {
        ...
        if ($id == "") {
            ...
            $hospdiscount = HospitalManagements::where('id', '=', $hospitalList->hospital_id)->first();
            ...
            if ($request->isMethod('post')) {
                ...
                $hospital_reg = 'Hospital-Reg' . rand(1111, 9999);
                ...
                $createhospital->created_by = Auth::guard('admin')->user()->id;
                $createhospital->hospital_id = Auth::guard('admin')->user()->id;
                $createhospital->save();
                DB::commit();
                return redirect('admin/paitent-list')->with('success_message', $message);
            }
        } else {
            $title = "Edit ";
            ...
            if ($request->isMethod('post')) {
                ...
                $hospitalList = Admin::where('id', Auth::guard('admin')->user()->id)->first();
                $hospitaldel = HospitalManagements::where('id', '=', $hospitalList->hospital_id)->first();
                ...
                $getcommission = DB::table('commission_reqistation_amount')->where('admin_type', 'Hospital')->first();
                ...
                $data_insert_to_wallet_healthcard = array(
                    'user_type' => 'Hospital',
                    'hospital_id' => Auth::guard('admin')->user()->id,
                    'hospital_name' => $hospitalList->name,
                    'admin_per' => '50',
                ...
                DB::table('wallet_hospital')->insert($data_insert_to_wallet_healthcard);
                ...
                if ($check_inserted_id != null) {
                    $total_healthcard_amount = $check_inserted_id->hospital_commision + $city_commison;
                    $total_amount = $check_inserted_id->total + $city_commison;
                    $inser_total_withdraw_trasection = array(
                        'total' => $total_amount,
                        'hospital_commision' => $total_healthcard_amount,
                        'updated_at' => date('Y-m-d H:i:s'),
                    ...
                    $inser_total_withdraw_trasection = array(
                        ...
                        'hospital_commision' => $city_commison,
                ...
                if ($check_inserted_id != null) {
                    $total_healthcard_amount = $check_inserted_id->hospital_commision + $ditrict_commison;
                    ...
                    $inser_total_withdraw_trasection = array(
                        ...
                        'hospital_commision' => $ditrict_commison,
                        'level_income_value' => 0,
                ...
                if ($check_inserted_id != null) {
                    $total_healthcard_amount = $check_inserted_id->hospital_commision + $state_commison;
                    ...
                    $inser_total_withdraw_trasection = array(
                        ...
                        'hospital_commision' => $state_commison,
        ...
        $state = DB::table('states')->where('status', '1')->get();
        return view('admin.hospital.edit_paitent_details')->with(compact('title', 'EditPaitent'));
    }
    ...
    public function viewPaitentbill($id)
    {
        ...
        return view('admin.hospital.view_paitent_bill')->with(compact('InvoiceData'));
    }

    public function HositalInovieCustomerWise()
    {
        $paitentlist = PatientDetails::where('member_id', '=', Auth::guard('admin')->user()->member_id)->where('add_paitent_details.paitent_discharge_date', '!=', null)->where('hospital_id','!=',0)->get();
        return view('admin.hospital.paitent_discharge_list')->with(compact('paitentlist'));
    }

    public function HospitalWisePaitentList($id)
    {
        $createhospital = HospitalManagements::find($id);
        $admin =  Admin::where('hospital_id', '=', $id)->first();
        $paitentlist = PatientDetails::where('hospital_id', $admin['id'])->get();

        $todayDate = Carbon::now()->format('Y-m-d');
        $thisMonth = Carbon::now()->format('m');
        $thisYear = Carbon::now()->format('Y');

        $totalPatientDetails = PatientDetails::where('hospital_id', $admin['id'])->count();
        $todayPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereDate('created_at', $todayDate)->count();
        $thisMonthPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereMonth('created_at', $thisMonth)->count();
        $thisYearPatientDetails = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereYear('created_at', $thisYear)->count();

        $totalcompanycommission = PatientDetails::where('hospital_id', $admin['id'])->sum('company_commission_amount');
        $totalDaycompanycommission = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereDate('created_at', $todayDate)->sum('company_commission_amount');
        $totalMonthscompanycommission = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereMonth('created_at', $thisMonth)->sum('company_commission_amount');
        $totalYearcompanycommission = DB::table('add_paitent_details')->where('hospital_id', $admin['id'])->whereYear('created_at', $thisYear)->sum('company_commission_amount');

        return view('admin.hospital.hospital_wise_paitent_list')->with(compact(
            'paitentlist',
            'totalPatientDetails',
            'todayPatientDetails',
            'thisMonthPatientDetails',
    ...
    public function HospitalCommissionReciver(Request $request, $id)
    {
        $createhospital = HospitalManagements::find($id);
        ...
        if ($request->isMethod('post')) {
            ...
            if($createhospital['total_commission_hicl'] >=$data['amount_recive'])
            {
                ...
                $hsopitalCommisionReicver->hospital_id = $createhospital['id'];
                ...
                return redirect('admin/hospital-List')->with('error_message', $message);
            }
        }
        $hospitalCommisisonHistory = HospitalCommisionReocrd::where('hospital_id', $id)->get();
        return view('admin.hospital.hospital_commission_record')->with(compact('createhospital', 'hospitalCommisisonHistory'));
    }

    public function PaymentReciptComp()
    {
        $paymentRecpitOfComm = DB::table('hospital_commision_record_admin')->where('hospital_id',Auth::guard('admin')->user()->hospital_id)->get();
           $hosdelt = Admin::where('id',Auth::guard('admin')->user()->id)->first();
                $hospitlaTotalComm = HospitalManagements::where('hospital_id',$hosdelt->hospital_id)->first();
               return view('admin.hospital.hospital_payment_recipt_histroy')->with(compact('paymentRecpitOfComm','hospitlaTotalComm'));
    }

    public function HospitalMultiImages(Request $request,$id)
    {
        $addMultiImages = HospitalManagements::find($id);
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->hasFile('images')) {
                ...
                foreach ($images as $key => $image) {
                    ...
                    $largeImagePath = 'admin_assets/uploads/hospital/large/' . $imageName;
                    $mediumImagePath = 'admin_assets/uploads/hospital/medium/' . $imageName;
                    $smallImagePath = 'admin_assets/uploads/hospital/small/' . $imageName;
           ...
           $gethospitalWiseDetails = HospitalManagements::with('images','hospitalMoreDetails','specializationHospitalList')->where('hospital_id', $id)->first();
          $gethospitalWiseDetails1 = DB::table('add_more_details')->where('hospital_id', $id)->first();
          $specialization = DB::table('specialization_hospitals')->get();
        return view('admin.hospital.hospital_more_details')->with(compact('gethospitalWiseDetails','gethospitalWiseDetails1','specialization'));
    }
    ...
    public function HospitalMoreDetails(Request $request ,$id)
    {

              $hospitaladd = AddMoreDetails::where('hospital_id',$id)->count();
        if ($request->isMethod('post')) {
            $data = $request->all();
            $rules = [
            ...
            $this->validate($request, $rules, $customMessages);
              if($hospitaladd > 0){
                ...
                $AddMoreDetails = AddMoreDetails::where('hospital_id',$id)->first();
                ...
                if ($request->hasFile('video')) {
                    ...
                    if ($video_tmp->isValid()) {
                        ...
                        $videoPath = 'admin_assets/uploads/hospital_video/';
            ...
            return redirect('admin/Hospital-More-Details/'.$id);
        }
    }

    public function HospitalAdditionalDetails(Request $request)
    {
        $adminshosp = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $addMultiImages = HospitalManagements::where('hospital_id',$adminshosp['hospital_id']);
        $message = "Your Images Has Been Uploaded Successfully";
        if ($request->isMethod('post')) {
            $data = $request->all();
            if ($request->hasFile('images')) {
                ...
                foreach ($images as $key => $image) {
                    ...
                    $image = new AddMultiImages;
                    $image->image = $imageName;
                    $image->hospital_id = $adminshosp['hospital_id'];
                    $image->save();
                }
            }
            return redirect()->back()->with('success_message', 'Product Image has been Updated successfully!');
           ...
           $gethospitalWiseDetails = HospitalManagements::with('images','hospitalMoreDetails')->where('hospital_id', $adminshosp['hospital_id'])->first();
          $gethospitalWiseDetails1 = DB::table('add_more_details')->where('hospital_id', $adminshosp['hospital_id'])->first();
          $specialization = DB::table('specialization_hospitals')->get();
        return view('admin.hospital.hospital_more_details_byhosiptal')->with(compact('gethospitalWiseDetails','gethospitalWiseDetails1','specialization'));
    }

    public function HospitalDetailsAdditional(Request $request)
    {
        $adminshosp = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $hospitaladd = AddMoreDetails::where('hospital_id',$adminshosp['hospital_id'])->count();
        if ($request->isMethod('post')) {
            $data = $request->all();
            $rules = [
                'video'  => 'mimes:mp4,mov,ogg | max:20000'
            ...
            $this->validate($request, $rules, $customMessages);
                ...
                $AddMoreDetails = AddMoreDetails::where('hospital_id',$adminshosp['hospital_id'])->first();
                ...
                $AddMoreDetails->higlight_point_1 = $data['higlight_point_1'];
                $AddMoreDetails->higlight_point_2 = $data['higlight_point_2'];
                if (!empty($data['emergency_care'])) {
                    $AddMoreDetails->emergency_care = $data['emergency_care'];
                } else {
                    $AddMoreDetails->emergency_care = "No";
                }
                if (!empty($data['operation_theatre'])) {
                    $AddMoreDetails->operation_theatre = $data['operation_theatre'];
                } else {
                    $AddMoreDetails->operation_theatre = "No";
                }
                if (!empty($data['qualified_doctor'])) {
                    $AddMoreDetails->qualified_doctor = $data['qualified_doctor'];
                } else {
                    $AddMoreDetails->qualified_doctor = "No";
                }
                if (!empty($data['ambulance'])) {
                    $AddMoreDetails->ambulance = $data['ambulance'];
                } else {
                    $AddMoreDetails->ambulance = "No";
                }
                $AddMoreDetails->hospital_id = $adminshosp['hospital_id'];
                $AddMoreDetails->save();
                }else {
            ...
            return redirect('admin/Hospital-Additional-Details/');
        }
     }

public function AddHospitalSpecializa(Request $request ,$id)
        {
            if ($request->isMethod('post')) {
                $data = $request->all();
                ...
                $SpecializationWiseHospitalDetails->hospital_id =$id;
        ...
        public function HospitalDetailsSpecialization(Request $request)
        {
           $adminshosp = Admin::where('id',Auth::guard('admin')->user()->id)->first();
           $adddoctor = HospitalManagements::where('hospital_id',$adminshosp['hospital_id'])->first();
            if ($request->isMethod('post')) {
                $data = $request->all();
                $SpecializationDetails = SpecializationHospitals::where('specialization_name',$data['specialization_name'])->first();

                $SpecializationWiseHospitalDetails = new SpecializationWiseHospitalDetails;
                $SpecializationWiseHospitalDetails->hospital_id =$adddoctor['hospital_id'];
                $SpecializationWiseHospitalDetails->specialization_id =$SpecializationDetails->id;
                $SpecializationWiseHospitalDetails->specialization_name =$data['specialization_name'];
                $SpecializationWiseHospitalDetails->save();
        ...
        public function OnlineBookApplist()
        {
            if (Auth::guard('admin')->user()->type == "Hospital") {
                $AppoimentList = BooKAppointent::where('hospital_id', Auth::guard('admin')->user()->hospital_id)->where('appointment_cancel', '=', 'pending')->get();
            }else{
            ...
            return view('admin.hospital.online_book_appointent_list')->with(compact('AppoimentList'));
        }

        public function Editonlineappointent(Request $request,$id)
        {
            ...
            if ($request->isMethod('post')) {
                ...
                if($data['appointment_cancel']=='Accept'){
                    ...
                    //  });
                     ...
                     return redirect('admin/Online-Appointent-List/')->with('success_message',$message);
            ...
            return view('admin.hospital.Edit_online_appointent')->with(compact('AppoimentList'));
        }
        ...
        public function OnlineBookaccpetedlist()
        {
            if(Auth::guard('admin')->user()->type == "Hospital")
            {
                $AppoimentaccepetedList = BooKAppointent::where('hospital_id',Auth::guard('admin')->user()->hospital_id)->where('appointment_cancel','=','Accept')->get();
            }
            else
            {
                $getuserid = DB::table('admins')->where('id',Auth::guard('admin')->user()->id)->first();
            ...
            return view('admin.hospital.online_appointent_accpeted_list')->with(compact('AppoimentaccepetedList'));
        }
        public function OnlineBookrejectedlist()
        {
            if(Auth::guard('admin')->user()->type == "Hospital")
            {
                $AppoimentrejectedList = BooKAppointent::where('hospital_id',Auth::guard('admin')->user()->hospital_id)->where('appointment_cancel','=','Reject')->get();
            ...
            return view('admin.hospital.online_appointent_rejected_list')->with(compact('AppoimentrejectedList'));
        }
